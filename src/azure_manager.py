    ...
    def get_latest_commit(self):
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.log_error(f"[{self.class_name}] Azure API request failed: {response.status_code} - {response.text}")
            ...
            self.logger.log_error(f"[{self.class_name}] Network error while connecting to Azure: {e}")
            print(f"DEBUG: Exiting function due to network error.")  # 🔥 Add explicit debug
            return None  # ✅ Ensure function exits immediately
